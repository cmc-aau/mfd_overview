...
FL16S <- read_csv(paste0(data.path, "/mfd_analyse_metadata.csv")) %>%
  filter(startsWith(fieldsample_barcode, "MFD"),
         fieldsample_barcode %in% mfd_db$fieldsample_barcode) %>%
  left_join((mfd_db %>%
               mutate(mfd_sampletype_update = mfd_sampletype,
                      mfd_areatype_update = mfd_areatype,
                      mfd_hab1_update = mfd_hab1) %>%
               select(fieldsample_barcode, latitude, longitude, mfd_sampletype_update, mfd_areatype_update, mfd_hab1_update)),
            by = "fieldsample_barcode") %>%
  mutate(mfd_sampletype = if_else(!is.na(mfd_sampletype_update), mfd_sampletype_update, mfd_sampletype),
         mfd_areatype = if_else(!is.na(mfd_areatype_update), mfd_areatype_update, mfd_areatype),
         mfd_hab1 = if_else(!is.na(mfd_hab1_update), mfd_hab1_update, mfd_hab1)) %>%
  select(-mfd_sampletype_update, -mfd_areatype_update, -mfd_hab1_update)
...
MG.map.BH.1km <- ggplot(data = world) + 
  geom_sf(fill = wes_palette("Darjeeling2")[1]) +
  geom_point(data = (mfd_db), aes(x = longitude, y = latitude, color = mfd_sampletype), 
             size = .75, alpha = 1, shape = 16) +
...
MG.map.DK.1km <- ggplot(data = world) + 
  geom_sf(fill = wes_palette("Darjeeling2")[1]) +
  geom_point(data = (mfd_db), aes(x = longitude, y = latitude, color = mfd_sampletype), 
             size = .75, alpha = 1, shape = 16) +
  geom_polygon(data = as.data.frame(geom(as.polygons(dk.1km))),
...
to.slice <- mfd_db %>%
  ...
  mutate(TO.MFDO1 = paste0(mfd_sampletype, ", ", mfd_areatype, ", ", mfd_hab1),
         TO.MFDO1 = if_else(TO.MFDO1%in%c("Soil, Natural, Sclerophyllous scrub",
                                          "Soil, Natural, Temperate heath and scrub"),
                            "Soil, Natural, Temperate heath and scrub",
                            TO.MFDO1)) %>%
  filter(!TO.MFDO1 %in% c("Water, Subterranean, Freshwater",
                          "Water, Urban, Sandfilter",
                          "Soil, Urban, Other",
                         "Sediment, Urban, Other",
                         "Soil, Urban, Roadside",
...
ggsave(combined.plots, filename = paste0(results.path, "/sampling_pixels_10km_v2.png"), width = 16, height = 9, dpi = "retina")
ggsave(combined.plots, filename = paste0(results.path, "/sampling_pixels_10km_v2.svg"), width = 16, height = 9, dpi = "retina")
```
